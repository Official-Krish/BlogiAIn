...
export async function generateChatResponse(apiKey: string, blogContent: string, userQuery: string, chatHistory: ChatCompletionMessageParam[], blogTitle: string) {
	const openai = new OpenAI({ apiKey: apiKey });
	const PROMPT_PREFIX_CHAT_COMPLETION = "You are a helpful assistant that answers questions about blog content. Do not answer questions unrelated to the blog content."
